...
class Tribe__Events__Tickets__Woo__Main extends Tribe__Events__Tickets__Tickets {
	...
	public function save_ticket( $event_id, $ticket, $raw_data = array() ) {
		...
		if ( empty( $ticket->ID ) ) {
			$save_type = 'create';
			//JER default
			$ticket->description = (empty( $ticket->description)) ? "General Admission for Members or guests of Members." : $ticket->description;
			...
			//JER update_post_meta( $ticket->ID, '_virtual', 'yes' ); //virtual is now a flag for non-tickets (eg parking)
...
//$this->spit("ticket->start_date is $ticket->start_date ");
		...
		if (  $raw_data['dayof_price']  >0 ){
			if ( empty( $dayof->ID ) && $edit_tix == false) {
				...
				if ( isset( $ticket->end_date ) ) {
					//$this->spit("ticket end date: $ticket->end_date");
			...
			//$('#ticket_end_meridian select option[value="am"]').prop('selected',true);
					...
					//$this->spit("new end_date = $dayof->end_date");
					...
					//$this->spit("this should be midnite: $tmp");
			...
			update_post_meta( $dayof->ID, '_stock', 100 );

...
class TribeWooTicketsEmail extends WC_Email {
	...
	function get_content_html() {
		...
		$attendees = array();
		$homepath = ABSPATH; //JER
		file_put_contents('/var/www/production/qrerrors.txt', "\n" . date('Y-m-d H:i:s'). ": homepath is  $homepath", FILE_APPEND);
		include_once $homepath . 'wp-content/lib/phpqrcode/qrlib.php';//JER TODO FIXME! Path
		foreach ( $query->posts as $post ) {
			$sec_code = get_post_meta( $post->ID, $wootickets->security_code, true ); //JER
			$attendees[] = array( 'event_id'      => get_post_meta( $post->ID, $wootickets->atendee_event_key, true ),
			                      ...
			                      'ticket_id'     => $post->ID,
			                      'security_code' => $sec_code //JER
			);
		
			//JER generate QR code file
			$qrfile = $homepath . "qr/$sec_code.png";
			if( !file_exists($qrfile) ) {
		 		file_put_contents('/var/www/production/qrerrors.txt', "\n" . date('Y-m-d H:i:s'). ": qrfile is $homepath - $qrfile", FILE_APPEND);
				QRcode::png($sec_code, $qrfile); // creates file

...
$this->spit("get_event_costs $event->ID");
		...
		$parsed_costs = array();
//JER desperation
		return tribe_get_event_meta( $event->ID, '_EventCost', true );
//JER
...
return $costs;//JER!!!
		...
		if ( $relevant_costs['min'] == $relevant_costs['max'] ) {
			...
			//JER $formatted = $relevant_costs['min'] . _x( ' - ', 'Cost range separator', 'the-events-calendar' ) . $relevant_costs['max'];
...
public function spit($sometext){
	$file = 'tixdebug.log';
	$today = date("Y-m-d H:i:s") . " Cost_Utils"; 

...
class TribeWooTickets extends TribeEventsTickets {
	...
	public function save_ticket( $event_id, $ticket, $raw_data = array() ) {
		...
		if ( empty( $ticket->ID ) ) {
			...
			spit('created ticket: $ticket->ID');
		} else {
			spit('editing a ticket');
		...
		if ( isset( $raw_data['parking_pass'] ) ){
			...
			$parking_pass->description = 'Guarantee a spot in our Parking Lot';

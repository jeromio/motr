...
function clean_contact_send($atts) {
	...
	$msgtype = $_POST['clean_contact_type']; //JER value to be set by buttons in javascript
	$to_email = ($msgtype ? $msgtype . "@motorcomusic.com" : $to_email);
...
//JER	if($from_email_set = get_option('clean_contact_from_email')) {
//JER		if(clean_contact_valid_email($from_email_set)) {
//JER			$from_email = $from_email_set;
//JER			$from = $from_email_set;
...
$from = $to_email;//JER
	...
	$to = $to_email;// JER  '"' . addslashes(get_bloginfo('name'))  . '" ' . "<$to_email>";
		
//JER	if(clean_contact_valid_email($cc)) 
$headers[] = "CC: {$cc}";
	...
	if(get_option('clean_contact_akismet') == 1 and clean_contact_akismet($body,$subject,$from_email,$from_name)) {
		...
		$ret = wp_mail($to, $subject, $body, implode("\n",$headers));
		return $ret;
	}
}
...
function clean_contact_strings() {
	...
	$strings['str_clean_contact_email'] = __('Email');

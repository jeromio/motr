...
//spit("JER get_cart_from_session");
			// Load cart session data from session
			foreach ( $this->cart_session_data as $key => $default ) {
				//spit("JER looping thru cart_session_data");
			...
			if ( is_array( $cart ) ) {
				foreach ( $cart as $key => $values ) {
					//spit("JER loop thru cart values");
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function check_cart_coupons() {
			foreach ( $this->applied_coupons as $code ) {
				//spit("JER looping in check_cart_coupons");
		...
		public function get_cart_item_quantities() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				//spit("JER looping in get_cart_item_quantities");
		...
		public function check_cart_item_validity() {
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
//spit("JER looping in check_cart_item_validity");
				$_product = $values['data'];
		...
		public function check_cart_item_stock() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				//spit("JER looping in check_cart_item_stock");
				$_product = $values['data'];
...
//spit("JER looping in get_item_data");
					if ( '' === $value )
						continue;
			...
			if ( $other_data && is_array( $other_data ) && sizeof( $other_data ) > 0 ) {
				...
				foreach ( $other_data as $data ) {
					//spit("JER get_item_data other_data");
		...
		public function get_cart() {
			if ( ! did_action( 'wp_loaded' ) ) {
				//spit("JER get_cart not wp_loaded");
				//$this->get_cart_from_session();
		...
		private function get_cart_for_session() {
			...
			if ( $this->get_cart() ) {
				foreach ( $this->get_cart() as $key => $values ) {
					//spit("JER looping in get_cart_for_session");
		...
		public function get_tax_totals() {
			...
			foreach ( $taxes as $key => $tax ) {
				//spit("JER looping in get_tax_totals");
		...
		public function get_cart_item_tax_classes() {
			...
			foreach ( WC()->cart->get_cart() as $item ) {
				//spit("JER looping in get_cart_item_tax_classes");
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_totals() {
			 ...
			 */
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
//spit("JER looping in calculate_totals");
				$_product = $values['data'];
			 ...
			 */
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
//spit("JER looping again in calculate cart totals (items)");
				$_product = $values['data'];
				 ...
				 */
					...
					foreach ( array_keys( $this->taxes + $discounted_taxes ) as $key ) {
						//spit("JER looping again in calculate - tax rows");
				 ...
				 */
					...
					// Tax rows - merge the totals we just got
					foreach ( array_keys( $this->taxes + $discounted_taxes ) as $key ) {
						//spit("JER looping in calculate again2 - tax rows merge");
						$this->taxes[ $key ] = ( isset( $discounted_taxes[ $key ] ) ? $discounted_taxes[ $key ] : 0 ) + ( isset( $this->taxes[ $key ] ) ? $this->taxes[ $key ] : 0 );
					}
				}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_shipping_packages() {
			...
			foreach ( $this->get_cart() as $item ) {
				//spit("JER looping in get_shipping_packages");
		...
		public function needs_shipping() {
			...
			if ( $this->cart_contents ) {
				foreach ( $this->cart_contents as $cart_item_key => $values ) {
					//spit("JER looping in needs_shipping");
					$_product = $values['data'];
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function check_customer_coupons( $posted ) {
			if ( ! empty( $this->applied_coupons ) ) {
				foreach ( $this->applied_coupons as $code ) {
					//spit("JER looping in check_customer_coupons");
					$coupon = new WC_Coupon( $code );
					...
					if ( $coupon->is_valid() ) {
						...
						if ( $coupon->usage_limit_per_user > 0 ) {
							...
							foreach ( $check_emails as $check_email ) {
								//spit("JER looping in coupon thing - check_emails");
		...
		public function add_discount( $coupon_code ) {
			...
			if ( $this->applied_coupons ) {
				foreach ( $this->applied_coupons as $code ) {
					//spit("JER looping thru applied_coupon codes");
					$coupon = new WC_Coupon( $code );
		...
		public function get_coupons( $deprecated = null ) {
			...
			foreach ( $this->get_applied_coupons() as $code ) {
				//spit("JER looping within get_coupons");
				$coupon = new WC_Coupon( $code );
				$coupons[ $code ] = $coupon;
			}
		...
		public function get_discounted_price( $values, $price, $add_totals = false ) {
			...
			if ( ! empty( $this->coupons ) ) {
				...
				foreach ( $this->coupons as $code => $coupon ) {
					//spit("JER looping in get_discounted_price");
		...
		public function calculate_fees() {
			...
			if ( ! empty( $this->fees ) ) {
				foreach ( $this->fees as $fee_key => $fee ) {
					//spit("JER looping in calculate_fees");
					...
					if ( $fee->taxable ) {
						...
						if ( ! empty( $fee_taxes ) ) {
							...
							foreach ( array_keys( $this->taxes + $fee_taxes ) as $key ) {
								//spit("JER looping again in fees - merge tax");
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_taxes_total( $compound = true, $display = true ) {
			...
			foreach ( $this->taxes as $key => $tax ) {
				//spit("JER loopin in get_taxes_total (tax)");
				if ( ! $compound && WC_Tax::is_compound( $key ) ) continue;
				$total += $tax;
			}
			foreach ( $this->shipping_taxes as $key => $tax ) {
				//spit("JER looping again in get taxes (shipping_taxes)");
				if ( ! $compound && WC_Tax::is_compound( $key ) ) continue;
				$total += $tax;
			}
			if ( $display ) {

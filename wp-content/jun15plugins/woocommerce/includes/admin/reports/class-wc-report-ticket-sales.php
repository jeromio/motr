 ...
 * @version     2.1.0
 */
class WC_Report_Ticket_Sales extends WC_Admin_Report {

	public $chart_colours      = array();
	public $ticket_ids        = array();
	public $ticket_ids_titles = array();

	/**
	 * Constructor
	 */
	public function __construct() {
		if ( isset( $_GET['ticket_ids'] ) && is_array( $_GET['ticket_ids'] ) ) {
			$this->ticket_ids = array_filter( array_map( 'absint', $_GET['ticket_ids'] ) );
		} elseif ( isset( $_GET['product_ids'] ) ) {
			$this->ticket_ids = array_filter( array( absint( $_GET['ticket_ids'] ) ) );
		}
	}
	...
	public function get_chart_legend() {
		...
		$total_sales = $this->get_order_report_data( array(
			...
			'where_meta' => array(
				...
				array(
					'type'       => 'order_item_meta',
					'meta_key'   => array( '_ticket_id', '_variation_id' ),
					'meta_value' => $this->ticket_ids,
					'operator'   => 'IN'
				)
			),
			'query_type'   => 'get_var',
	...
	public function output_report() {
		...
		$current_range = ! empty( $_GET['range'] ) ? sanitize_text_field( $_GET['range'] ) : 'All';

		if ( ! in_array( $current_range, array( 'custom', 'year', 'all', 'month', '7day' ) ) )
			$current_range = 'all';
	...
	public function get_chart_widgets() {
		...
		$widgets[] = array(
			'title'    => '',
			'callback' => array( $this, 'tickets_widget' )
		);
	...
	public function current_filters() {
		...
		foreach ( $this->ticket_ids as $product_id ) {
	 ...
	 * Tickets selection
	 */
	public function tickets_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Tickets Search', 'woocommerce' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>

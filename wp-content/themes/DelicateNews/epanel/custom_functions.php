...
if ( ! function_exists( 'get_thumbnail' ) ) {
	function get_thumbnail($width=100, $height=100, $class='', $alttext='', $titletext='', $fullpath=false, $custom_field='', $post='') {
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			//echo "post $post->ID \n<br>";
			//Jeremy removed this $thumb_array['use_timthumb'] = false;
			...
			//echo "fullpath is $et_fullpath[0] \n<br>";
		}
...
if ( ! function_exists( 'print_thumbnail' ) ) {
	function print_thumbnail($thumbnail = '', $use_timthumb = true, $alttext = '', $width = 100, $height = 100, $class = '', $echoout = true, $forstyle = false, $resize = true, $post='', $et_post_id = '' ) {
		...
		//echo "print_thumbnail $thumbnail \n<BR>";
		...
		if ( false ) { //Jeremy has_post_thumbnail( $et_post_id ) ) {
			//Jeremy Removed $thumb_array['use_timthumb'] = false;
			...
			//echo ".... .... ... image size set to $image_size_name \n<br> and therefore et_size is: $et_size \n<br>";
			...
			//echo "new thumbnail is $thumbnail \n<br>";
			//echo "width is $et_attachment_image_attributes[1] \n<br>";
			//echo "height is $et_attachment_image_attributes[2] \n<br>";
		} else {
		...
		if ( false === $forstyle ) {
			...
			$output = $forstyle . " stupid " . $thumbnail;
		}
...
if ( ! function_exists( 'show_page_menu' ) ){
	function show_page_menu($customClass = 'nav clearfix', $addUlContainer = true, $addHomeLink = true){
		...
		//Modified by JER to alter MyAccount/SignUp menu
		//file_put_contents("debugmenu",  " tryna change menu \n$page_menu\n", FILE_APPEND | LOCK_EX);
		if ( !is_user_logged_in() ) {
			$page_menu = wp_list_pages("sort_column=".et_get_option($shortname.'_sort_pages')."&sort_order=".et_get_option($shortname.'_order_page')."&".$strdepth."&exclude=8,10,".$exclude_pages."&title_li=&echo=0");
		
			//file_put_contents("debugmenu",  "User is not logged in!\n", FILE_APPEND | LOCK_EX);
			/*$MyAccount = '<li class="page_item page-item-10 current_page_item"><a href="http:\/\/motr.co/my-account/">My Account</a></li>';
			$SignUp = '<li class="page_item page-item-10 current_page_item"><a href="http:\/\/motorcomusic.com/my-account/">Sign Up/a></li>';
			$TR = str_replace ( $MyAccount, $SignUp , $page_menu );	
			$test = is_array($page_menu);
			//file_put_contents("debugmenu",  "array? $test\n", FILE_APPEND | LOCK_EX);
			*/
		} else {
			$page_menu = wp_list_pages("sort_column=".et_get_option($shortname.'_sort_pages')."&sort_order=".et_get_option($shortname.'_order_page')."&".$strdepth."&exclude=13479,".$exclude_pages."&title_li=&echo=0");
		
		}
		
		//file_put_contents("debugmenu",  "Did we change menu?\n $page_menu\n", FILE_APPEND | LOCK_EX);
		if ($addUlContainer) echo('<ul class="'.$customClass.'">');
			if (et_get_option($shortname . '_home_link') == 'on' && $addHomeLink) { ?>
				<li <?php if (is_front_page() || is_home()) echo('class="current_page_item" style="z-index:999;"') ?>><a href="<?php echo esc_url( home_url() ); ?>"><?php _e('Home',$themename); ?></a></li>
			<?php };

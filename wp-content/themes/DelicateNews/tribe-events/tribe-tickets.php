...
if ( ! class_exists( 'TribeEventsTickets' ) ) {
	abstract class TribeEventsTickets {
		...
		function __construct() {
			...
			//JER tryna add sell tickets option
			add_filter( 'post_row_actions', array( $this, 'selltix_row_action' ) );
			// Front end
		...
		public final function generate_attendees_PDF( $tickets_list ) {
			...
			foreach ( $tickets_list as $ticket ) {
				...
				$pdf->Image('http://motorcomusic.com/images/MOTORCO_logo.png',110,10);	

				$pdf->SetDrawColor( 88, 166, 205 );
				$pdf->SetFillColor( 88, 166, 205 );
				$pdf->Rect( 15, 75, 180, 40, 'F' );
				...
				$pdf->SetXY( 120, 165 );
				// Jeremy added QR Codes
				include_once $this->parentPath . 'lib/phpqrcode/qrlib.php';
				QRcode::png($ticket['security_code'], 'QRFile.png'); // creates file
				$pdf->Image( 'QRFile.png'); 
				$pdf->Write( 4, $ticket['security_code'] );

				$pdf->Rect( 15, 220, 180, 15, 'F' );
		...
		//JER with some crazy tix selling action
		public function selltix_row_action( $actions ) {
			global $post;
			...
			if ( $post->post_type == TribeEvents::POSTTYPE ) {
				...
				$url = add_query_arg( array( 'post_type' => TribeEvents::POSTTYPE,
				                             'page'      => 'tickets-sell',//$this->attendees_slug,
				                             'event_id'  => $post->ID ), admin_url( 'edit.php' ) );

				$actions['tickets_sell'] = sprintf( '<a title="%s" href="%s">%s</a>', __( 'Sell tickets to this event', 'tribe-events-calendar' ), esc_url( $url ), __( 'Attendees', 'tribe-events-calendar' ) );
			}
			return $actions;
		}
